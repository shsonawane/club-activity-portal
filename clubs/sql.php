<?php
$servername = "sql111.byethost33.com";
$username = "b33_21691966";
$password = "8390360997";
$dbname = "b33_21691966_clubs";
...
function createAccount($id, $cname, $dept, $phone, $email, $password, $type) {
    $stmt = $GLOBALS['conn']->prepare("INSERT INTO users VALUES (?,?,?,'-','-','-',?,'-',?,'-',?,?,0);");
    $stmt->bind_param("sssssss", $id, $cname, $dept, $phone, $email, $password, $type);
    $stmt->execute();
...
function updateAccount($id, $cname, $dept, $pres, $vicepres, $about, $phone, $whatsapp, $email, $web) {
    $stmt = $GLOBALS['conn']->prepare("UPDATE `users` SET `name`=?, `dept`=?, `president`=?, `vice-president`=?, `about`=?, `phone`=?, `whatsapp`=?, `email`=?, `web`=? WHERE `idusers`=?;
    ");
    $stmt->bind_param("ssssssssss", $cname, $dept, $pres, $vicepres, $about, $phone, $whatsapp, $email, $web, $id);
    $flg = $stmt->execute();
    $stmt->close();
    return $flg;
}
